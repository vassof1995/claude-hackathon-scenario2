...
- ADR placed at `docs/adr/ADR-003-reporting-db-read-replica.md` per task instruction. This
  differs from the repo convention (`decisions/NNNN-title.md`); cross-referenced in the ADR.
...
3. **Ran a parallel workflow** (run id `wf_28ac2311-eb4`) to author + adversarially verify the
   ...
   - `docs/adr/ADR-003-reporting-db-read-replica.md` — Multi-AZ + read replica decision.
